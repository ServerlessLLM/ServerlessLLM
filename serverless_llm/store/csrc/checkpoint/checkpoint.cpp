...
std::unordered_map<std::string, uint64_t> SaveTensors(
  ...
  // Some tensors may share the same data, so we need to record the data to avoid duplication
...
std::unordered_map<std::string, torch::Tensor> RestoreTensors(
    const std::unordered_map<std::string, std::tuple<std::vector<int64_t>, std::vector<int64_t>, std::string>>& meta_state_dict,
  ...
  for (const auto& [device, tensor_offset] : tensor_device_offsets) {
    for (const auto& p : tensor_offset) {
      ...
      if (memory_base_address.find(device) != memory_base_address.end()) {
        ...
        uint64_t offset =
            reinterpret_cast<uint64_t>(base_address) + p.second;
        ...
        // std::cerr << name << " " << sizes << " " << strides << " " << dtype << std::endl;
        if (p.second == 0 && handled_memory.find(base_address) == handled_memory.end()) {
          ...
          // std::cerr << "Tensor " << name << " is restored to device " << device << std::endl;
        } else {
          torch::Tensor real_tensor = torch::from_blob(
...
std::unordered_map<int, void*> AllocateCudaMemory(const std::unordered_map<int, size_t>& tensor_sizes) {
...
std::unordered_map<int, std::string> GetCudaMemoryHandles(const std::unordered_map<int, void*>& memory_ptrs) {
  ...
  for (const auto& p : memory_ptrs) {
    ...
    memory_handles[device] = std::string(reinterpret_cast<const char*>(&handle), sizeof(cudaIpcMemHandle_t));
  }
  return memory_handles;
}

std::unordered_map<int, std::vector<std::string>> GetCudaMemoryHandles(const std::unordered_map<int, std::vector<void*>>& memory_ptrs) {
  ...
  for (const auto& p : memory_ptrs) {
    ...
    for (const auto& ptr : ptrs) {
      cudaIpcGetMemHandle(&handle, ptr);
      handles.push_back(std::string(reinterpret_cast<const char*>(&handle), sizeof(cudaIpcMemHandle_t)));
    }
