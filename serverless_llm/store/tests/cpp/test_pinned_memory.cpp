...
TEST_F(PinnedMemoryPoolTest, InitializePinnedMemoryPool) {
  auto pinned_memory_pool = std::make_unique<PinnedMemoryPool>(mem_pool_size, chunk_size);
...
TEST_F(PinnedMemoryPoolTest, ConcurrentAllocateDeallocate) {
  ...
  for (int i = 0; i < 100; ++i) {
    futures.push_back(std::async(std::launch::async, [size, &pinned_memory_pool]() -> int {
      std::vector<char*> buffers;
...
TEST_F(PinnedMemoryTest, AllocatePinMemory) {
  size_t size = 1024;
  auto pinned_memory_pool = std::make_shared<PinnedMemoryPool>(mem_pool_size, chunk_size);
  std::unique_ptr<PinnedMemory> pinned_memory = std::make_unique<PinnedMemory>();
