...
def _get_live_gpu_info() -> dict:
    ...
    try:
        ...
        logger.error(f"Failed to retrieve live GPU information: {e}", exc_info=True)
...
def benchmark_static_hardware(test_network: bool = False) -> dict:
    ...
    if test_network:
        logger.warning("Running network bandwidth test. This can take a minute...")
...
def _get_static_gpu_info() -> dict:
    ...
    try:
        ...
        logger.error(f"Failed to retrieve static GPU information: {e}", exc_info=True)
        gpus_info = {"status": "error", "message": str(e)}
    return gpus_info
...
def _benchmark_disk_bandwidth(file_size_mb: int = 100, num_iterations: int = 3) -> tuple:
    ...
    try:
        ...
        logger.info(f"Disk benchmark results: Write={avg_write/1e6:.2f} MB/s, Read={avg_read/1e6:.2f} MB/s")
