...
def validate_model_identifier(model_identifier: str) -> tuple[str, str]:
    ...
    if len(parts) != 2:
        raise ValidationError("Model identifier must be in format 'model_name:backend'")
...
def validate_model_name(model_name: str) -> None:
    ...
    if ".." in model_name or "/" in model_name or "\\" in model_name:
        raise ValidationError("Model name cannot contain path traversal sequences")
...
def validate_node_ip(ip_address: str) -> None:
    ...
    if ip.is_loopback:
        ...
        if not os.getenv("SLLM_DEV_MODE", "false").lower() == "true":
            raise ValidationError("Loopback addresses not allowed in production")
...
def validate_storage_path(storage_path: str) -> None:
    ...
    try:
        ...
        if not path.exists():
            raise ValidationError(f"Storage path does not exist: {storage_path}")
        
        # Check if it's a directory
        if not path.is_dir():
            raise ValidationError(f"Storage path is not a directory: {storage_path}")
        
        # Check if writable
        if not os.access(path, os.W_OK):
            raise ValidationError(f"Storage path is not writable: {storage_path}")
