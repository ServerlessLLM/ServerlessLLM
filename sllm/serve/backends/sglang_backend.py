...
class SGLangBackend(SllmBackend):
    def __init__(self, model: str, backend_config: Dict[str, Any] = None):
        ...
        self.cache = OptimizedCache(
            cfg.get("cache_size", 500), cfg.get("cache_ttl", 1800)
        )
        ...
        logger.info(
            f"SGLangBackend init: server_url={self.server_url}, use_native={self.use_native}"
        )
    ...
    async def _init_endpoint(self) -> bool:
        ...
        try:
            logger.info(
                f"🔗 Connecting to SGLang Runtime endpoint: {self.server_url}"
            )
    ...
    async def stream_generate(
        ...
        try:
            ...
            if self.mode == SGLangMode.NATIVE_ENDPOINT and self.endpoint:
                ...
                try:
                    logger.debug(
                        "🚀 Using native endpoint streaming generation..."
                    )
                    state = await asyncio.to_thread(
                        self.sglang_functions["simple_chat"].run, msg=prompt
                    )
                    ...
                    if hasattr(state, "response"):
                        ...
                        hasattr(state, "_variables")
                        and "response" in state._variables
                    ):
                    ...
                    logger.warning(
                        f"Native streaming generation failed: {e}, falling back to HTTP streaming"
                    )
                    async for chunk in self._http_stream_generate(
                        prompt, params
                    ):
                        generated_text += chunk
                        yield chunk
    ...
    async def _http_stream_generate(
            ...
            if (
                ...
                try:
                    ...
                    if text.startswith("ProgramState(") or not text:
                        logger.warning(
                            "Native SGLang returned abnormal format, falling back to HTTP API"
                        )
                        ...
                        logger.debug(
                            f"✅ Native generation successful: {text[:50]}..."
                        )

                except Exception as e:
                    logger.warning(
                        f"Native SGLang failed: {e}, falling back to HTTP"
                    )
                    text = await self._http_generate(prompt, params)
                    method = "http_api_fallback"
            else:
    ...
    def get_current_tokens(self) -> int:
        ...
        return sum(
            ...
            for req in self.metrics.values()
        )
        ...
        try:
            ...
            async def do_prefill(input_ids: List[int]):
                ...
                if hasattr(self.endpoint, "prefill"):
                    ...
                    logger.warning(
                        "SGLang endpoint does not support prefill method"
                    )

            # Gather all requests
            await asyncio.gather(*(do_prefill(ids) for ids in data))
            logger.info(
                f"Successfully prefilled {len(data)} contexts to SGLang cache"
            )
    ...
    def get_backend_stats(self) -> Dict[str, Any]:
        ...
        uptime = time.time() - (
            min(m.start_time for m in self.metrics.values())
            if self.metrics
            else time.time()
        )
        ...
        stream_requests = sum(
            1 for m in self.metrics.values() if getattr(m, "stream", False)
        )
        ...
        return {
            ...
            "success_rate": self.successful / self.total_requests
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "native_endpoint_available": self.mode
            == SGLangMode.NATIVE_ENDPOINT,
        }
