...
class SGLangBackend(SllmBackend):
    ...
    def _setup_model_config(self) -> None:
        ...
        if load_format is not None:
            ...
            self.model_path = self.backend_config.get(
                "pretrained_model_name_or_path"
            )
        else:
            storage_path = os.getenv("STORAGE_PATH", "./models")
            self.model_path = os.path.join(
                storage_path, "sglang", self.model_name
            )
    ...
    def _setup_engine_config(self) -> None:
        ...
        self.mem_fraction_static = self.backend_config.get(
            "mem_fraction_static"
        )
    ...
    def _has_scheduler_with_requests(self) -> bool:
        ...
        return (
            self.engine
            and hasattr(self.engine, "scheduler")
            and hasattr(self.engine.scheduler, "running_reqs")
        )
        ...
        async with self.status_lock:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"SGLang engine initialized successfully in {init_time:.2f} seconds"
                    )
    ...
    async def generate(self, request_data: Dict[str, Any]):
        ...
        try:
            ...
            sampling_params = {
                "max_new_tokens": request_data.get(
                    "max_tokens", request_data.get("max_new_tokens", 100)
                ),
    ...
    async def encode(self, request_data: Dict[str, Any]):
        ...
        try:
            ...
            text = request_data.get(
                "input",
                request_data.get("text", request_data.get("prompt", "")),
            )
            if not text:
            ...
            result = await self.engine.async_encode(
                prompt=text, image_data=image_data
            )
        ...
        try:
            ...
            return [
                ...
                + (
                    ...
                    if hasattr(req, "outputs") and req.outputs
                    else []
                )
    ...
    async def resume_kv_cache(self, request_datas: List[List[int]]) -> None:
        ...
        try:
            ...
            for request_data in request_datas:
                if not (
                    request_data
                    and isinstance(request_data, list)
                ...
                if (
                    self.engine
                    and hasattr(self.engine, "tokenizer")
                    and hasattr(self.engine.tokenizer, "decode")
                ):
                    prompt_text = self.engine.tokenizer.decode(
                        request_data, skip_special_tokens=True
                    )
                else:
            ...
            if constructed_inputs:
                ...
                logger.info(
                    f"KV cache resumed for {len(constructed_inputs)} token sequences"
                )
        ...
        if self.engine:
            try:
                shutdown_method = getattr(
                    self.engine, "shutdown", None
                ) or getattr(self.engine, "stop", None)
                if shutdown_method:
