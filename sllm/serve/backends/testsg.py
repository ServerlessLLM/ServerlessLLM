...
async def test_sglang_backend_integration():
    ...
    try:
                ...
                try:
                    ...
                    if hasattr(stream_result, "__aiter__"):
                        ...
                        async for chunk in stream_result:
                            ...
                            if (
                                chunk_count >= 10
                            ):  # Collect first 10 chunks only
                                break

                        stream_content = "".join(chunks)
                        print(
                            f"✅ Streaming generation successful: {chunk_count} chunks"
                        )
                        print(f"🌊 Streaming content: {stream_content[:80]}...")
                    else:
                        print(
                            "❌ Streaming generation returned incorrect format"
                        )
        ...
        if "error" not in http_result:
            http_method = http_result.get("sglang_info", {}).get(
    ...
    try:
        ...
        async for chunk in stream_result:
            if first_chunk_time is None:
                ...
                print(
                    f"⚡ Time to first chunk: {first_chunk_time - start_time:.3f}s"
                )
        ...
        print(
            f"   - Characters per second: {len(total_content)/total_time:.1f}"
        )
