...
def select_rel(passages, query, r):
    def compute_relevance_scores(query_encoding, document_encodings, k):
        scores = torch.matmul(
    ...
    query_encoding = (
        client_relevance.embeddings.create(
            input=[query], model=relevance_expert
        )
        .data[0]
        .embedding
    )
    ...
    passage_encoding_res = client_relevance.embeddings.create(
