...
if patch -p2 --dry-run -d "$SGLANG_PATH" < "$PATCH_FILE"; then
    echo "SGLang patch is not applied. Applying the patch now..."
    # Apply the patch
    patch -p2 -d "$SGLANG_PATH" < "$PATCH_FILE"
    echo "Patch applied successfully."
else
    echo "SGLang patch has already been applied. Skipping..."
fi